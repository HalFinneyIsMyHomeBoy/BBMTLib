...
NOSTR_KEYPAIR1=$("$BUILD_DIR/$BIN_NAME" nostr-keypair)
NOSTR_KEYPAIR2=$("$BUILD_DIR/$BIN_NAME" nostr-keypair)
NOSTR_KEYPAIR3=$("$BUILD_DIR/$BIN_NAME" nostr-keypair)
...
echo "Starting Relay..."
"$BUILD_DIR/$BIN_NAME" relay "$PORT" "$USENOSTR" "$NOSTRRELAY" &
PID0=$!
...
"$BUILD_DIR/$BIN_NAME" keygen "$SERVER" "$SESSION_ID" "$CHAIN_CODE" "$PARTY1" "$PARTIES" "$PUBLIC_KEY2" "$PRIVATE_KEY1" "$SESSION_KEY" "$USENOSTR" "$NOSTRRELAY" "$NOSTR_PUBLIC_KEY1" "$NOSTR_PRIVATE_KEY1" &
PID1=$!

echo "Starting Keygen for PARTY2..."
"$BUILD_DIR/$BIN_NAME" keygen "$SERVER" "$SESSION_ID" "$CHAIN_CODE" "$PARTY2" "$PARTIES" "$PUBLIC_KEY1" "$PRIVATE_KEY2" "$SESSION_KEY" "$USENOSTR" "$NOSTRRELAY" "$NOSTR_PUBLIC_KEY2" "$NOSTR_PRIVATE_KEY2" &
PID2=$!

echo "Starting Keygen for PARTY3..."
"$BUILD_DIR/$BIN_NAME" keygen "$SERVER" "$SESSION_ID" "$CHAIN_CODE" "$PARTY3" "$PARTIES" "$PUBLIC_KEY1" "$PRIVATE_KEY2" "$SESSION_KEY" "$USENOSTR" "$NOSTRRELAY" "$NOSTR_PUBLIC_KEY3" "$NOSTR_PRIVATE_KEY3" &
PID3=$!
